...
class ApproveVerificationController extends Controller
{
								...
								)
	{
		...
		$this->module_view_folder    = "support.verification_approve";
	...
	public function index(Request $request)
	{
		...
		$this->arr_view_data['page_title']                    = 'Approved '.str_plural($this->module_title);
	...
	public function load_approve_data(Request $request)
	{
		...
		$obj_data = DB::table($request_table)
					...
					->where($prefixed_request_table.'.status','=','1')
        ...
        if(count($UserData)>0)
        {
            ...
            {
            	$built_view_href   = $this->module_url_path.'/view_approve_data/'.base64_encode($row->id);
	...
	public function view_approve_data($id)
	{
		$arr_data = [];
		$id = base64_decode($id);
		...
		$this->arr_view_data['page_title']                = 'View Approved '.str_singular($this->module_title);
		$this->arr_view_data['previous_page_url']         = $this->support_url_path."/verification/approve";
		$this->arr_view_data['previous_page_title']       = "Approved Verification Requests";
		$this->arr_view_data['module_title']              = str_plural($this->module_title);
		$this->arr_view_data['module_url_path']           = $this->module_url_path;
		$this->arr_view_data['support_panel_slug']        = $this->support_panel_slug;
