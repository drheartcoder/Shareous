      ...
      <form name="validation-form" id="validate_form" method="POST" class="form-horizontal" action="{{$module_url_path}}/update/{{$id}}" enctype="multipart/form-data" files ="true">
        {{csrf_field()}}
         ...
         <div class="form-group col-lg-11">
           <label class="col-sm-4 col-lg-3 control-label" for="page_title" style="text-align: right;">Query Type <i class="red">*</i> </label>
           <div class="col-sm-8 col-lg-5 controls">
            <input type="text" name="query_type" class="form-control" value="{{isset($querytype['query_type']) ? $querytype['query_type'] :'' }}" data-rule-required="true" data-rule-maxlength="255"  placeholder="Property Type Name ">
            <span class='error help-block'>{{ $errors->first('query_type') }}</span>
          </div>          
        </div>
      <div class="form-group col-lg-11">
       <div class="col-sm-6 col-sm-offset-3 col-lg-6 col-lg-offset-3">
        ...
        <a href="{{url($admin_panel_slug.'/query_type')}}" type="button" class="btn btn-cancel">Cancel</a>
      </div>
    </div>
