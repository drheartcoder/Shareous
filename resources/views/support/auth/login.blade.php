...
<head>
    ...
    <title>{{ $site_settings['site_name'] or '' }} Support Login</title>
    ...
    <link rel="stylesheet" href="{{ url('/web_support/assets/font-awesome/css/font-awesome.min.css') }}">
    ...
    <style type="text/css">
        ...
        .btn-lgn
        {
            background-color: #A6733E!important;
            border-radius: 10px!important;
        }

        .btn-lgn:hover
        {
            background-color: #986530!important;
        }
...
<body class="login-page" style='background-image: url("{{url('/web_support/images/5.jpg')}}") !important;background-repeat:repeat-x;background-size: 100%;' id="background">

    <!-- BEGIN Main Content -->
    <div class="login-wrapper">
        <!-- BEGIN Login Form -->
        <form name="form-login" id="form-login" method="POST" action="{{ url($support_panel_slug.'/validate_login') }}" @if( Session::has('success_password') || Session::has('error_password')) style="display:none" @else style="display:block;" @endif>
            {{ csrf_field() }}
        ...
        <form id="form-forgot" action="{{ url($support_panel_slug.'/forgot_password') }}" method="post" @if(Session::has('success_password') || Session::has('error_password')) style="display:block;" @else style="display:none;" @endif>
           {{ csrf_field() }}
...
<script>window.jQuery || document.write('<script src="{{ url('/web_support/assets/jquery/jquery-2.1.4.min.js') }}"><\/script>')</script>
<script src="{{ url('/web_support/assets/bootstrap/js/bootstrap.min.js') }}"></script>
<script src="{{ url('/web_support/assets/jquery-slimscroll/jquery.slimscroll.min.js') }}"></script>
<script src="{{ url('/web_support/assets/jquery-cookie/jquery.cookie.js') }}"></script>
<script type="text/javascript" src="{{ url('/web_support/assets/jquery-validation/dist/jquery.validate.min.js') }}"></script>
<script type="text/javascript" src="{{ url('/web_support/assets/jquery-validation/dist/additional-methods.min.js') }}"></script>
<script type="text/javascript" src="{{ url('/web_support/assets/chosen-bootstrap/chosen.jquery.min.js') }}"></script>

<!--flaty scripts-->
<script src="{{ url('/web_support/js/flaty.js') }}"></script>
<script src="{{ url('/web_support/js/flaty-demo-codes.js') }}"></script>
<script src="{{ url('/web_support/js/validation.js') }}"></script>
...
<script type="text/javascript">
    ...
    var spinner_html = "<img src='{{url('/web_support/images/spin.gif')}}'/>";
