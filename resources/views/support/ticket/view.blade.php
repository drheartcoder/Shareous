         ...
         <div class="row">
            <div class="col-sm-6">
               ...
               @if( $arr_user['booking_details'] != null && !empty( $arr_user['booking_details'] ) )
                ...
                <?php
                    ...
                    $paid_amount = isset($arr_user['booking_details']['total_amount']) && $arr_user['booking_details']['total_amount'] != "" ? number_format($arr_user['booking_details']['total_amount'], 2, '.', '') : '0';
                    ...
                    $refund_amount = isset($arr_user['booking_details']['refund_amount']) && $arr_user['booking_details']['refund_amount'] != "" ? number_format($arr_user['booking_details']['refund_amount'], 2, '.', '') : '0';
                ?>

                <input type="hidden" id="paid_amount" value="{{ $paid_amount }}">
                <input type="hidden" id="refund_amount" value="{{ $refund_amount }}">
               ...
               <div class="panel panel-default">
                  ...
                  <div class="panel-body">
                     <div class="form-group">
                        ...
                        @if( $refund_amount != 0 )
                        <div class="form-group col-sm-12">
                           <label class="col-lg-4 control-label">Total Refund Amount:</label>
                           <div class="col-lg-8">
                              <div class="col-lg-12">
                                 {!! '<i class="fa fa-inr" aria-hidden="true"></i> '.$refund_amount !!}
                              </div>
                           </div>
                        </div>
                        @endif
            ...
            <div class="col-sm-6">
               ...
               @endif
                ...
                @php
                    $booking_id = isset($arr_user['booking_id']) && $arr_user['booking_id'] != "" ? $arr_user['booking_id'] : '';
                @endphp
                @if($query_type_id == 0 && count($arr_user['booking_details']['transaction_details']) > 0 )
                   <div class="panel panel-default">
                      <div class="panel-heading font-bold">Refund to Guest</div>
                      <div class="panel-body">
                         <div class="form-group">
                            <div class="form-group col-sm-12">
                               <label class="col-lg-4 control-label">Refund Amount:</label>
                               <div class="col-lg-8">
                                  <input type="text" id="txt_refund_amount" name="txt_refund_amount" class="form-control" data-rule-required="true" data-rule-maxlength="255" placeholder="Amount" maxlength="12" />
                                  <div class="error" id="err_txt_refund_amount" style="display: none;"></div>
                               </div>
                            </div>
                            <div class="form-group col-sm-12">
                               <label class="col-lg-4 control-label">Process Refund:</label>
                               <div class="col-lg-8">
                                  <a  href="javascript:void(0)" onclick='refund("{{ base64_encode($booking_id) }}")'  class="btn btn-success" id="btn_refund_amount"><i class="fa fa-check" ></i> Accept & Refund</a>
                                  <a href="javascript:void(0)" class="btn btn-success loader" id="btn_process_refund" style="display: none; font-size: 6.8px; width: 88px;"><i class="fa fa-spinner fa-spin fa-3x fa-fw"></i></a>
                               </div>
                            </div>
                         </div>
                      </div>
                ...
                <form id="verification_form" method="post" name="verification_form">
                   ...
                   <div class="form-group">
                      <div class="col-lg-12">
                         <!-- <a  href="javascript:void(0)" onclick='accept("{{ base64_encode($arr_user['id']) }}")' class="btn btn-success btn-custom"><i class="fa fa-reply" ></i> Reply</a> -->
                         <a  href="{{ url($module_url_path) }}/reply/{{ base64_encode($arr_user['id']) }}" class="btn btn-success btn-custom"><i class="fa fa-reply" ></i> Reply</a>
                         <a  href="javascript:void(0)" onclick='close_ticket("{{ base64_encode($arr_user['id']) }}")'  class="btn btn-danger"><i class="fa fa-times"></i> Close Ticket</a>
                         @if(isset($support_level) && $support_level=="L2" || $support_level=="L3")
                         <a  href="javascript:void(0)" onclick='change_level("{{ base64_encode($arr_user['id']) }}")' class="btn btn-success" style="background-color: darkslateblue !important;"><i class="fa fa-paper-plane"></i> Change Level</a>
                         @endif
...
<script type="text/javascript">
   ...
   function refund(id)
   {
     ...
     var return_amount = paid_amount - refund_amount;
     console.log( return_amount +'  '+ paid_amount +'  '+ refund_amount);

     if(txt_refund_amount == '')
     {
       $("#err_txt_refund_amount").html("Please enter amount").delay(5000).fadeOut('slow');
       $("#err_txt_refund_amount").show();
       $("#txt_refund_amount").focus();
     }
     else if(txt_refund_amount == 0)
     {
       $("#err_txt_refund_amount").html("Please enter valid amount").delay(5000).fadeOut('slow');
       $("#err_txt_refund_amount").show();
       $("#txt_refund_amount").focus();
     }
     else if( (parseFloat(txt_refund_amount)) > (parseFloat(paid_amount)))
     {
       $("#err_txt_refund_amount").html("Refund amount can't be greater than Total Paid amount").delay(5000).fadeOut('slow');
       $("#err_txt_refund_amount").show();
       $("#txt_refund_amount").focus();
     }
     else if( (parseFloat(txt_refund_amount)) <= (parseFloat(return_amount)))
     {
       ...
       $("#err_txt_refund_amount").html("Some amount is already refunded. Cross check entered amount").delay(5000).fadeOut('slow');
       $("#err_txt_refund_amount").show();
       $("#txt_refund_amount").focus();
     }
   }
