...
class SocialiteManager extends Manager implements Contracts\Factory
{
    ...
    {
        if (strpos($config['redirect'], 'twitter') !== false) {
            $arr_twitter             = get_twitter_credential();
            $config['client_id']     = $arr_twitter['twitter_client_id'];
            $config['client_secret'] = $arr_twitter['twitter_client_secret'];
        }

        if (strpos($config['redirect'], 'facebook') !== false) {
            $arr_facebook            = get_facebook_credential();
            $config['client_id']     = $arr_facebook['facebook_client_id'];
            $config['client_secret'] = $arr_facebook['facebook_client_secret'];
        }

        if (strpos($config['redirect'], 'google') !== false) {
            $arr_google              = get_google_credential();
            $config['client_id']     = $arr_google['google_client_id'];
            $config['client_secret'] = $arr_google['google_client_secret'];
        }
